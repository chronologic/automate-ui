...
import { AssetType, IDecodedTransaction, Status } from '../../../types';
...
class TransactionStatus extends React.Component<ITransactionStatusProps, any> {
  private explorers: IExplorers = {
    ...
    [AssetType.Polkadot]: new Map<number, string>([[1, 'https://polkascan.io/polkadot/transaction/']]),
  };
  ...
  public render() {
    ...
    const statusText =
      this.props.status === Status.Error && this.props.error ? `${status} - ${this.props.error}` : status;
    ...
    return (
      <div>
        ...
        <div className="bx--row row-padding">
          <TextInput id="status" className="bx--col-xs-12" labelText="Status" disabled={true} value={statusText} />
        </div>
        <div className="bx--row row-padding">{this.renderTransactionHash()}</div>
      </div>
    );
  }
      ...
      if (explorerUrl) {
        return (
          <FormItem>
            ...
            <Link href={explorerUrl} className="bx--text-input bx--col-xs-12" target="_blank">
