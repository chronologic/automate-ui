...
export default class SummarySection extends React.Component<ISummarySectionProps> {
  public render() {
    ...
    if (timeCondition && timeConditionTZ) {
      formattedDate = moment.tz(timeCondition, timeConditionTZ).format('DD/MM/YYYY h:mma') + ' ' + timeConditionTZ;
    }

    const notSupportedNetwork = (
      <span className="bx--tag bx--tag--experimental">{`Network with id ${chainId} is not supported ✖`}</span>
    );
    const supportedNetwork = <span className="bx--tag bx--tag--community">{`${networkName} ✔`}</span>;
        ...
        <div className="bx--col-xs-6 main-section">
          ...
          <div className="schedule-summary">
            <div className="schedule-summary_party">
              {signedSender && (
                <img src={toDataUrl(signedSender)} alt="sender" className="schedule-summary_party_blockie" />
              )}
            ...
            <div className="schedule-summary_details">
              ...
              <div className="schedule-summary_details_network">
                {signedSender ? (isNetworkSupported ? supportedNetwork : notSupportedNetwork) : ''}
              </div>
              <div className="schedule-summary_details_arrow" />
              <div className="schedule-summary_details_condition">
                when
                <br />
                <SelectiveDisplay first={6} last={4} text={signedSender} /> balance of {conditionalAsset.name} is{' '}
                {'>= '}
                ...
                {timeScheduling && timeCondition && formattedDate && (
                  <>
                    ...
                    >
                      {moment(timeCondition).format('DD/MM/YYYY h:mma')} {moment.tz.guess()}
                    </Tooltip>
                    <br />
                  </>
                )}
            ...
            <div className="schedule-summary_party">
              {signedRecipient && (
                <img src={toDataUrl(signedRecipient)} alt="recipient" className="schedule-summary_party_blockie" />
              )}
              <div className="schedule-summary_party_address">
  ...
  private getNonceInfo(): JSX.Element {
    ...
    let status = <span className="bx--tag bx--tag--experimental">✖ - nonce lower than account nonce</span>;

    if (signedNonce === senderNonce) {
      status = <span className="bx--tag bx--tag--community">✔</span>;
    } else if (signedNonce > senderNonce) {
      status = <span className="bx--tag bx--tag--private">! - There's a gap between nonces</span>;
    }

    return (
      <>
        Transaction nonce: <b className="font-weight-600">{signedNonce}</b> {status}
      </>
    );
  }
  ...
  private getTxCostInfo(): JSX.Element {
    ...
    if (maxTxCost.gt(senderBalance)) {
      status = <span className="bx--tag bx--tag--private">! - sender balance too low to cover tx cost</span>;
    } else {
      status = <span className="bx--tag bx--tag--community">✔ sender balance enough to cover tx cost</span>;
    }
  ...
  private getGasInfo(): JSX.Element {
    ...
    return (
      <>
        Gas price: <b className="font-weight-600">{bigNumberToString(signedGasPrice, 9, 0)} Gwei</b> Gas limit:{' '}
        <b className="font-weight-600">{bigNumberToString(signedGasLimit, 0, 0)}</b>{' '}
      </>
    );
  }
}
