...
const AssetSelector: React.FC<IAssetSelectorProps> = ({ active, selectedSymbol, selectedChainId, onClick }) => {
  const assetNodes = assets.map((asset) => {
    const isSelected = asset.symbol === selectedSymbol && asset.chainId === selectedChainId;
          ...
          <div className={asset.main ? 'asset-center' : 'asset-left'}>
            {isSelected ? (
              <object
                ...
                height={asset.size}
              >
                <param name="src" value={asset.imgUrl.replace('.svg', '_a.svg')} />
              </object>
            ) : (
              <embed type="image/svg+xml" src={asset.imgUrl} height={asset.size} width={asset.size} />
            )}
  ...
  return (
    <div className={cn('bx--row asset-selector', active ? ' main-section-blue' : '')}>
      <div className="bx--col-xs-6 main-section">
        <h2 className="asset-selector-title">Select the asset you want to schedule a transaction for</h2>
